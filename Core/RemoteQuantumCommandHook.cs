...
internal sealed class RemoteQuantumCommandHook : IDisposable
{
    ...
    private const int DuplicateCommand = 2;
    ...
    private const int DuplicateItemOffset = 0x820;
    ...
    public string Arm()
    {
        ...
        code.AddRange(new byte[sizeof(int)]);
        code.AddRange(new byte[] { 0x83, 0xF8, DuplicateCommand, 0x0F, 0x84 });
        var duplicateJump = code.Count;
        code.AddRange(new byte[sizeof(int)]);
        ...
        var duplicateOffset = code.Count;
        AddStoreInt32RipRelative(code, root, CommandOffset, 0);
        AddLoadRipRelative(code, root, FrameOffset, new byte[] { 0x48, 0x8B, 0x0D });
        AddLoadRipRelative(code, root, SelectedItemOffset, new byte[] { 0x48, 0x8B, 0x15 });
        ...
        AddLoadRipRelative(code, root, FrameOffset, new byte[] { 0x48, 0x8B, 0x0D });
        AddLoadRipRelative(code, root, SelectedItemOffset, new byte[] { 0x48, 0x8B, 0x15 });
        AddCallAbsolute(code, _duplicateItem);
        AddStoreRipRelative(code, root, DuplicateItemOffset, new byte[] { 0x48, 0x89, 0x05 });
        code.AddRange(new byte[] { 0x48, 0x85, 0xC0, 0x0F, 0x84 });
        var duplicateFailedJump = code.Count;
        code.AddRange(new byte[sizeof(int)]);

        AddLoadRipRelative(code, root, FrameOffset, new byte[] { 0x48, 0x8B, 0x0D });
        AddLoadRipRelative(code, root, InventoryOwnerOffset, new byte[] { 0x48, 0x8B, 0x15 });
        AddLoadRipRelative(code, root, DuplicateItemOffset, new byte[] { 0x4C, 0x8B, 0x05 });
        code.AddRange(new byte[] { 0x41, 0xB9, 0x04, 0x00, 0x00, 0x00 }); // NewInventoryItemSource.Cheat
        ...
        AddStoreRipRelative(code, root, ResultOffset, new byte[] { 0x89, 0x05 });
        AddStoreInt32RipRelative(code, root, CompletedOffset, DuplicateCommand);
        var duplicateDoneJump = AddNearJump(code);

        var duplicateFailureOffset = code.Count;
        ...
        PatchNearJump(code, duplicateJump, duplicateOffset);
        PatchNearJump(code, unknownCommandJump, restoreOffset);
        PatchNearJump(code, rarityDoneJump, restoreOffset);
        PatchNearJump(code, missingOwnerJump, duplicateFailureOffset);
        PatchNearJump(code, duplicateFailedJump, duplicateFailureOffset);
        PatchNearJump(code, duplicateDoneJump, restoreOffset);
    ...
    public bool QueueDuplicate(long selectedItem)
    {
        ...
        return _session.WriteInt64(root + SelectedItemOffset, selectedItem) &&
               _session.WriteInt32(root + CompletedOffset, 0) &&
               _session.WriteInt32(root + ResultOffset, 0) &&
               _session.WriteInt64(root + DuplicateItemOffset, 0) &&
               _session.WriteInt64(root + InventoryOwnerOffset, 0) &&
               _session.WriteInt32(root + CommandOffset, DuplicateCommand);
    }
    ...
    public bool TryReadDuplicateCompletion(out bool completed, out bool succeeded, out long duplicateItem)
    {
        duplicateItem = 0;
        return TryReadCompletion(DuplicateCommand, out completed, out succeeded) &&
               _session.TryReadInt64(_trampoline.ToInt64() + DuplicateItemOffset, out duplicateItem);
    }
